...
fstatus_t platformInit(void *argv[]) {  //arg[0] is xclbin file and arg[1] is platform name. arg[2] is kernel name.
  ...
  // Check psl_server.dat is present
    ...
    if(argv[1] != NULL){
        ...
        target_device_name_pass = "_u250_xdma_201820_1";  //Don't know what to put here as default name!!!!!!!!!!!!!
    }
  ...
  if (!device_found) {
   ...
   // Create a compute context
    ...
    int status;  //Indicates whether the program binary for the device was loaded successfully or not.
...
// I believe the below portion should not be on platformInit.
